...
#Gir variabler nye navn

#Lager ny variabel med elektrisitet sitt bidrag 
df_elektrisitet <- df_elektrisitet %>%
  ...
  rename("konsumpris" = "Konsumprisindeks (2015=100)") %>%
  mutate(vekter= vekter/1000) %>%
  mutate(arligelektrisitet = vekter*konsumpris)

df_elektrisitet <- df_elektrisitet[c("dato","arligelektrisitet")] %>%
  mutate(var="arligelektrisitet")

#Lager en totalindeks
df_total <- df_total %>%
  rename(arligelektrisitet = "value") %>%
   mutate(var="arligelektrisitet")

df_total <- df_total %>%
  mutate(arligelektrisitet = arligelektrisitet/1000)
 
df_ferdig <- rbind(df_elektrisitet, df_total)
 
  
df_ferdig %>%
  ggplot(aes(x=dato, y=arligelektrisitet, color=var)) +
  geom_line() +
  labs(title = "Endring i KPI i forhold til strom",
       subtitle = "Prosentvis okning i KPI og okning i kraftpris") +
  theme_bw()
...
Figuren viser forbruk av strøm, inkludert nettleie gjennom årene. Økning i kraftprisen trekkes i stor grad opp totalindeksen. Når prisen og forbruket av strøm er høyt vil totalindeksen stige slik som vist på grafen.
